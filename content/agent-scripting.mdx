...
Scripts run through the web panel's **Scripts** page, which sends the
`scripts.run` control message to the embedded runtime:
...
```json
{
  ...
  "params": {
    "contextId": "ctx-1",
    "script": "[{\"action\":\"navigate\",\"url\":\"https://example.com\"}]"
  }
}
```

The runtime executes each step against a real browser context and returns
step-by-step results plus captured variables.
