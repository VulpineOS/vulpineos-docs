---
title: "MCP Browser Tools — 38 Tools for AI Agent Browser Control"
description: "VulpineOS MCP tools: browser control, reliability checks, human-like actions, annotated screenshots, credential, audio, and Android mobile bridge tools for AI browser agents."
---

# MCP Tools

VulpineOS provides a [Model Context Protocol](https://spec.modelcontextprotocol.io/) (MCP) server that exposes browser automation as tool calls. Each AI browser agent uses these tools to browse, and every action routes through the selected Vulpine browser context.
...
| `vulpine_scroll_into_view` | Scroll a selector or ref into view |
...
| `vulpine_element_status` | Inspect visibility, enabled state, text, and geometry for a selector or ref |
...
These tools expose stable VulpineOS feature surfaces. Open-source builds return graceful unavailable errors when a provider is not present; provider-enabled builds can attach implementations behind the same public tool names.
...
![Annotated screenshot with durable Vulpine Mark labels](/media/vulpine-mark-annotated.png)
...
Mobile bridge tools expose Android bridge sessions in open-source builds and can share the same public API surface with other provider-enabled deployments:
...
VulpineOS prepares a local agent workspace and routes browser work through its
MCP/Juggler tools. Foxbridge can also expose a CDP endpoint for compatible
clients that expect a Chrome DevTools Protocol URL:
...
- [Foxbridge CDP Proxy](/foxbridge) — embedded CDP server for compatible clients
