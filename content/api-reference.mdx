...
History rows also include `mode` so operators can distinguish standard extract, authenticated, and visual runs. Authenticated rows additionally expose `login_trace_id` when a persisted redacted login trace exists. When browser-backed evidence capture is retained, rows also expose `evidence_screenshot_path` with a tenant-scoped download route.
