...
description: "Vulpine API endpoint reference for extraction, visual workflows, audio capture and transcription, mobile sessions, async tasks, monitors, and account operations."
---

# Vulpine API Reference

Vulpine API is the hosted API for VulpineOS browser extraction. It uses real browser sessions when available, falls back to HTTP fetches where appropriate, and returns structured JSON with usage metadata.
...
This endpoint requires credential-provider and browser-session support in the deployment.
...
This endpoint requires browser integration in the deployment.
...
These endpoints require an audio-capture provider in the deployment. Audio capture metadata is persisted so completed or interrupted artifacts remain listable and downloadable after API restarts until retention cleanup removes them.
...
Current monitor scheduling enqueues recurring extraction jobs. Change detection, condition evaluation, and richer webhook change payloads may vary by deployment.
...
- [Vulpine API Features](/api-features) — hosted extraction surface
- [Source Availability](/source-availability) — open-source and extension boundaries
