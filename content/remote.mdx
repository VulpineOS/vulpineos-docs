---
title: "Remote Access — Deploy VulpineOS on a VPS"
description: "VulpineOS remote deployment: serve mode, access-key authentication, hosted web panel, and remote TUI connections."
---

# Remote Access — Deploy VulpineOS on a VPS

VulpineOS supports running the browser kernel on one machine and controlling it
from another. The served control plane exposes:

- **Web panel** — open the embedded browser dashboard in your local browser
...
Open the panel URL printed by `vulpineos serve` in your local browser. The panel
validates the access key through `/auth/check` and uses the same `/ws` endpoint
as the remote TUI.
...
The TUI behaves identically to local mode — all panels, controls, and agent
...
The browser login form validates explicit keys through `/auth/check`, and the
panel websocket authenticates with the same access key.
